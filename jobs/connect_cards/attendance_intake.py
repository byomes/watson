...
def _find_or_create_member(conn: sqlite3.Connection, name: str, campus: str, service_date: str) -> int:
    ...
    # gender/deacon default to the '--' blank convention since nothing here
    # knows either.
    conn.execute(
        """
        INSERT INTO members (name, campus_preference, first_visit_date,
                              active, partner, residency, gender, deacon)
    ...
    for name in names:
        try:
            ...
            if not _attendance_exists(conn, member_id, service_date):
                ...
                # the one exception to active's "disconnected/deceased are
                ...
                try:
                    status_row = conn.execute(
                        "SELECT name, active FROM members WHERE id = ?", (member_id,)
                    ).fetchone()
                    if status_row and status_row["active"] == "disconnected":
                        conn.execute(
                            "UPDATE members SET active = 'active' WHERE id = ?",
                            (member_id,),
                        )
