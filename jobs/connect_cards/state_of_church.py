...
def _members_excluded_counts(conn: sqlite3.Connection) -> dict:
    ...
    residency_counts = dict(
        conn.execute(
            "SELECT residency, COUNT(*) FROM members "
            "WHERE residency IN ('non-local', 'snowbird') GROUP BY residency"
        ).fetchall()
    )
    return {
...
def _ollama_synthesis(condensed: str, benchmarks_context: str) -> str | None:
    prompt = (
        ...
        "g. When you flag who may need pastoral attention (lapsed/occasional members, absences), frame "
        "it as follow-up for the congregation's shepherds (elders/deacons), not as something Dr. Bill "
        "personally needs to go do himself — e.g. 'worth a shepherd reaching out to...' rather than "
        "'you should reach out to...'.\n\n"
        "Also comment on engagement health (what the Consistent/Active/Occasional/Lapsed distribution "
        "reveals), areas of concern, and who may need shepherding attention. Do not include a summary "
        "paragraph at the end. Do not repeat yourself. "
