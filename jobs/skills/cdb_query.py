...
def _pattern_match(question: str, last_sun: str, weeks: list) -> str | None:
    ...
    if any(w in q for w in ['slipping', 'falling off', 'not coming', 'stopped coming', 'needs attention', 'shepherding', 'missing recently', 'fading', 'drifting', 'losing touch', 'falling away', 'at risk of leaving']):
        ...
        return (
            ...
            f"WHERE m.active NOT IN ('disconnected', 'deceased') AND m.name NOT LIKE '%CAMPUS%' AND m.name NOT LIKE '%SYSTEM%' AND m.name NOT LIKE '%TEST%' "
    ...
    if any(w in q for w in ["how many missed", "how many didn't", "how many were absent", "how many did not", "how many were missing", "miss count"]):
        return (
            ...
            f"WHERE m.active NOT IN ('disconnected', 'deceased') AND m.name NOT LIKE '%CAMPUS%' AND m.name NOT LIKE '%SYSTEM%' AND m.name NOT LIKE '%TEST%' AND m.id NOT IN ("
    ...
    if any(w in q for w in ['new member', 'new people', 'new person', 'joined',
        ...
        if 'this week' in q:
            ...
            return (
                ...
                f"WHERE active NOT IN ('disconnected', 'deceased') AND name NOT LIKE '%CAMPUS%' AND name NOT LIKE '%SYSTEM%' AND name NOT LIKE '%TEST%' AND created_at >= '{start.strftime('%Y-%m-%d')}' "
            ...
            return (
                f"SELECT name, email, phone, created_at FROM members "
                f"WHERE active NOT IN ('disconnected', 'deceased') AND name NOT LIKE '%CAMPUS%' AND name NOT LIKE '%SYSTEM%' AND name NOT LIKE '%TEST%' "
    ...
    if _deacon_m:
        ...
        if deacon_name:
            return (
                ...
                f"WHERE deacon LIKE '%{deacon_name}%' AND active NOT IN ('disconnected', 'deceased') "
    ...
    if _team_roster_m:
        ...
        if team_phrase:
            return (
                ...
                f"WHERE tm.team_name LIKE '%{team_phrase}%' AND m.active NOT IN ('disconnected', 'deceased') ORDER BY m.name"
            )
    ...
    if _person_teams_m:
        ...
        if name:
            return (
                ...
                f"FROM members m WHERE m.name LIKE '%{name}%' AND m.active NOT IN ('disconnected', 'deceased')"
            )
    ...
    if _member_deacon_m:
        ...
        if member_name:
            return f"SELECT name, deacon FROM members WHERE name LIKE '%{member_name}%' AND active NOT IN ('disconnected', 'deceased')"
    ...
    if _last_missed_m:
        ...
        if name:
            return (
                ...
                f") as last_missed "
                f"FROM members m "
                f"WHERE m.name LIKE '%{name}%' AND m.active NOT IN ('disconnected', 'deceased')"
            )
        ...
        if name:
            if name.endswith('s') and len(name) > 1 and not name.endswith('ss'):
                ...
                return (
                    ...
                    f"(name LIKE '%{name}%' OR name LIKE '%{name[:-1]}%') AND active NOT IN ('disconnected', 'deceased')"
                )
            return f"SELECT name, phone FROM members WHERE name LIKE '%{name}%' AND active NOT IN ('disconnected', 'deceased')"
    ...
    if 'email address' not in q and 'ip address' not in q:
        ...
        if _address_m and name:
            ...
            return f"SELECT name, address FROM members WHERE name LIKE '%{name}%' AND active NOT IN ('disconnected', 'deceased')"
    ...
    if _age_m:
        ...
        if name:
            return (
                ...
                f"FROM members WHERE name LIKE '%{name}%' AND active NOT IN ('disconnected', 'deceased') AND birthdate IS NOT NULL"
            )
    ...
    if _serving_m:
        ...
        if name:
            return (
                ...
                f"FROM members WHERE name LIKE '%{name}%' AND active NOT IN ('disconnected', 'deceased') AND started_serving_date IS NOT NULL"
            )
    ...
    if any(w in q for w in ['look up', 'find member', 'search for', 'who is', 'tell me about',
        ...
        if name:
            return (
                f"SELECT m.name, m.email, m.phone, m.partner, m.active, m.campus_preference, m.first_visit_date "
                f"FROM members m "
                f"WHERE m.name LIKE '%{name}%' AND m.active NOT IN ('disconnected', 'deceased') AND m.name NOT LIKE '%CAMPUS%' AND m.name NOT LIKE '%SYSTEM%' AND m.name NOT LIKE '%TEST%' "
                f"ORDER BY m.name"
            )
    ...
    if any(w in q for w in ['how many members', 'how many active', 'total members', 'member count',
        ...
        if any(w in q for w in ['how many', 'total', 'count']):
            return "SELECT COUNT(*) as total FROM members WHERE active NOT IN ('disconnected', 'deceased')"
        return "SELECT name, email, campus_preference FROM members WHERE active NOT IN ('disconnected', 'deceased') AND name NOT LIKE '%CAMPUS%' AND name NOT LIKE '%SYSTEM%' AND name NOT LIKE '%TEST%' ORDER BY name"
...
- members has columns: id, name, email, phone, campus_preference, partner, active, residency, deacon
