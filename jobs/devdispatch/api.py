...
def _notify_archive_classification(archive_id: int, project: str, title: str) -> None:
    """Ping Telegram with OK/Change Filing buttons after a live 'send to watson' archive.

    Informational only — the archive is already saved by the time this fires, so
    this is not a confirmation gate. Scoped to the live archive_session MCP path
    only — the nightly bulk export importer (jobs/session_archives/claude_export_import.py)
    calls storage.archive_session() directly, not this wrapper, so a 600-conversation
    bulk run doesn't flood Telegram with one message per conversation. Tapping
    Change Filing (or just replying to the message) reclassifies via the existing
        ...
        requests.post(
            ...
            json={
                ...
                "reply_markup": {"inline_keyboard": [[
                    {"text": "OK", "callback_data": f"arch_keep:{pending_id}"},
                    {"text": "Change Filing", "callback_data": f"arch_chg:{pending_id}"},
                ]]},
            },
            timeout=10,
        )
