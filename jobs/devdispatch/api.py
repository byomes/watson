...
_TOOLS = [
    ...
    },
    {
        "name": "get_job_output",
        "description": (
            "Inspect what a dispatched Claude Code job actually produced — "
            "read-only, never commits/pushes/modifies anything, and safe to "
            "call on a running, failed, or succeeded job. Returns the committed "
            "diff if the branch has commits ahead of main; otherwise the "
            "uncommitted worktree contents (git status + diff against HEAD + "
            "any new files); otherwise the raw session log tail if no worktree "
            "or code changes exist. Use this to see what happened when a job "
            "failed before opening a PR and there is nothing on GitHub to look "
            "at."
        ),
        "inputSchema": {
            "type": "object",
...
# get_job_output size caps — this all flows back to Claude.ai as a single JSON
# string over one MCP call, so keep it usable rather than dumping a whole
# repo's worth of diff. Truncation is always marked with an explicit
# "... [truncated, N more lines]" line so nothing looks silently complete.
_MAX_DIFF_LINES = 600          # cap on a committed-branch diff
_MAX_FILE_CONTENT_LINES = 200  # cap per untracked/new file shown inline
_MAX_INLINE_FILES = 25         # cap on how many untracked files we read at all
_MAX_LOG_TAIL_CHARS = 4000     # cap on the (ANSI-stripped) session log tail

# `claude logs <id>` emits a raw ANSI/TUI transcript (cursor moves, colour
# codes, OSC title sets, redraws) — see module docstring. These strip it down
# to whatever plain text survives so get_job_output can surface *something*
# instead of just noting it's unparseable. CSI sequences, OSC sequences
# (BEL- or ST-terminated), and lone two-char escapes, in that order.
...
def _truncate_lines(text: str, max_lines: int) -> str:
    """Return text unchanged if within max_lines, else keep the first
    max_lines and append an explicit truncation marker so the caller never
    mistakes a cut-off dump for the whole thing."""
...
def _session_log_tail(cli_id: str) -> str:
    """Best-effort readable tail of the background session's transcript.
    `claude logs <id>` returns a raw ANSI/TUI transcript (not parseable
    plain text — this is the same source check_claude_code_job declines to
    surface); here we actually fetch it, strip the escape sequences, and
    return whatever plain text survives, tail-capped. May be sparse or
    mostly spinner labels — that's the nature of the transcript, reported
    honestly rather than pretended to be structured output."""
    if not cli_id:
        return "(no cli_session_id recorded for this job — cannot fetch a session log)"
    try:
        result = subprocess.run(
            [_CLAUDE_BIN, "logs", cli_id], capture_output=True, text=True, timeout=30
        )
    except Exception as exc:
        return f"(could not run `claude logs {cli_id}`: {exc})"
    raw = result.stdout or result.stderr or ""
    cleaned = _strip_ansi(_redact(raw))
    # Collapse the runs of blank lines the transcript leaves behind once its
    # cursor-movement codes are stripped.
    cleaned = re.sub(r"\n{3,}", "\n\n", cleaned).strip()
    if not cleaned:
        return "(session log contained only terminal control sequences — no readable text to show)"
    if len(cleaned) > _MAX_LOG_TAIL_CHARS:
        cleaned = "... [truncated to last %d chars]\n" % _MAX_LOG_TAIL_CHARS + cleaned[-_MAX_LOG_TAIL_CHARS:]
    return cleaned


def _read_untracked_files(worktree: Path, porcelain: str) -> dict:
    """Content of untracked ("?? ") files listed in `git status --porcelain`,
    read straight off disk (they have no HEAD to diff against). Binary/oversize
    files are noted, not dumped."""
    files = {}
    shown = 0
    for line in porcelain.splitlines():
        ...
        if shown >= _MAX_INLINE_FILES:
            files["(more)"] = f"... additional untracked files not shown (cap {_MAX_INLINE_FILES})"
            break
        path = worktree / rel
        if not path.is_file():
            # A "?? somedir/" entry is an untracked directory — note it rather
            # than trying to read a directory as a file.
            files[rel] = "(untracked directory — not expanded)"
            shown += 1
            continue
        try:
            content = path.read_text(encoding="utf-8", errors="strict")
        except (UnicodeDecodeError, ValueError):
            files[rel] = "(binary or non-UTF-8 file — content omitted)"
            shown += 1
            continue
        except Exception as exc:
            ...
            shown += 1
            continue
        files[rel] = _truncate_lines(content, _MAX_FILE_CONTENT_LINES)
        shown += 1
    return files


def _get_job_output(job_id) -> dict:
    """Read-only inspection of what a dispatched job actually produced —
    the committed diff, the uncommitted worktree contents, or the raw
    session log, depending on what exists. Never commits, pushes, deletes,
    or modifies anything: every git call here is a read (status/diff/log/
    rev-list), and files are only read, never written. Safe on a running
    job (shows work-in-progress) and independent of the job's status
    field."""
    if job_id is None:
        return {"error": "job_id is required"}
    ...
    # Case 3a — no worktree on disk (never created, or torn down after a
    # successful finalize). Nothing to diff; fall back to the session log.
    if not worktree.is_dir():
        out["worktree"] = "absent"
        out["message"] = (
            "No worktree on disk for this job (never created, or already "
            "cleaned up after completion) — no code changes to show. Session "
            "log tail below (raw terminal transcript, best-effort)."
        )
        out["log_tail"] = _session_log_tail(cli_id)
        return out

    out["worktree"] = str(worktree)

    # How far ahead of main is this worktree's HEAD? (Committed work — the
    # session may have committed on its own, or a prior check finalized it.)
    ahead_proc = _run_git(["rev-list", "--count", "main..HEAD"], worktree)
    if ahead_proc.returncode != 0:
        # Don't fail the whole read over a rev-list hiccup — surface it and
        # still try to show the working-tree state below.
        out["rev_list_error"] = _redact(ahead_proc.stderr.strip())[:300]
        ahead = 0
    else:
        ahead = int((ahead_proc.stdout or "0").strip() or "0")
    out["commits_ahead_of_main"] = ahead

    # Case 1 — real commits on the branch. Return the diff + file-list summary.
    if ahead > 0:
        log_proc = _run_git(["log", "--oneline", "main..HEAD"], worktree)
        out["commits"] = _redact((log_proc.stdout or "").strip())

        stat_proc = _run_git(["diff", "--stat", "main...HEAD"], worktree)
        out["files_changed"] = _redact((stat_proc.stdout or "").strip())

        diff_proc = _run_git(["diff", "main...HEAD"], worktree)
        if diff_proc.returncode != 0:
            out["diff_error"] = _redact(diff_proc.stderr.strip())[:300]
        else:
            out["diff"] = _truncate_lines(_redact(diff_proc.stdout), _MAX_DIFF_LINES)
        out["message"] = (
            f"Branch has {ahead} commit(s) ahead of main. Showing the committed "
            "diff (git diff main...HEAD)."
        )
        return out

    # No commits ahead of main — look at the working tree itself.
    status_proc = _run_git(["status", "--short"], worktree)
    if status_proc.returncode != 0:
        out["message"] = "Could not read git status of the worktree."
        out["git_status_error"] = _redact(status_proc.stderr.strip())[:300]
        out["log_tail"] = _session_log_tail(cli_id)
        return out

    porcelain = status_proc.stdout or ""

    # Case 2 — worktree has uncommitted/staged changes but no commits. Show
    # the status, a diff of everything against HEAD (covers both staged and
    # unstaged tracked changes), and the content of untracked new files.
    if porcelain.strip():
        out["git_status"] = _redact(porcelain.strip())
        # `git diff HEAD` shows staged + unstaged changes to tracked files in
        # one pass — this is the "diff against the worktree's HEAD" the spec
        # asks for when something's staged, and also covers plain modified
        # files. Untracked files have no HEAD entry, so they're read below.
        tracked_diff = _run_git(["diff", "HEAD"], worktree)
        if tracked_diff.returncode == 0 and tracked_diff.stdout.strip():
            out["tracked_diff"] = _truncate_lines(_redact(tracked_diff.stdout), _MAX_DIFF_LINES)
        elif tracked_diff.returncode != 0:
            out["tracked_diff_error"] = _redact(tracked_diff.stderr.strip())[:300]

        untracked = _read_untracked_files(worktree, porcelain)
        if untracked:
            out["new_files"] = untracked
        out["message"] = (
            "Worktree has uncommitted changes but no commits ahead of main. "
            "Showing git status, a diff of tracked changes against HEAD, and "
            "the content of any new (untracked) files."
        )
        return out

    # Case 3b — worktree exists but is clean and even with main: nothing was
    # produced. Fall back to the session log.
    out["message"] = (
        "Worktree exists but is clean and even with main — no commits and no "
        "uncommitted changes, so there is no code to show. Session log tail "
        "below (raw terminal transcript, best-effort)."
    )
    out["log_tail"] = _session_log_tail(cli_id)
    return out
...
_TOOL_IMPLS = {
    ...
    "get_job_output": lambda args: _get_job_output(args.get("job_id")),
}
