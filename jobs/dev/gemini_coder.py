...
def _call_gemini(description: str, system_prompt_override: str | None = None) -> str:
    ...
    if system_prompt_override is not None:
        ...
        system_prompt = (
            ...
            "YOUR ONLY JOB: Write a plain English description of WHAT to build. Nothing else.\n\n"
            "ABSOLUTE PROHIBITIONS — your output must NEVER contain:\n"
            "- Code of any kind (Python, JavaScript, shell, SQL, or any other language)\n"
            "- File paths or directory names\n"
            "- Function names, method names, class names, or variable names\n"
            "- JSON, XML, YAML, or any structured data format\n"
            "- Technical implementation details, algorithms, or step-by-step instructions\n"
            "- Markdown formatting, bullet points, headers, or code fences\n\n"
            "You describe WHAT the feature should do in clear plain English sentences.\n"
            "Claude Code figures out HOW to build it. Watson coding rules are appended automatically.\n"
            "Do not explain your reasoning. Do not describe files or code structure.\n"
            "Write as if explaining the desired end result to a non-technical product owner.\n\n"
...
def request_debug(description: str) -> int:
    ...
    debug_system_prompt = (
        ...
        "running on a Linux server at /home/billyomes/watson.\n\n"
        "YOUR ONLY JOB: Write a plain English description of the problem and what needs to be fixed. Nothing else.\n\n"
        "ABSOLUTE PROHIBITIONS — your output must NEVER contain:\n"
        "- Code of any kind (Python, JavaScript, shell, SQL, or any other language)\n"
        "- File paths or directory names\n"
        "- Function names, method names, class names, or variable names\n"
        "- JSON, XML, YAML, or any structured data format\n"
        "- Technical implementation details, algorithms, or step-by-step fix instructions\n"
        "- Markdown formatting, bullet points, headers, or code fences\n\n"
        "Describe the reported problem and its observable symptoms in plain English.\n"
        "Describe WHAT behavior should be correct after the fix, not HOW to fix it.\n"
        "Claude Code figures out the root cause and implementation on its own.\n"
        "Do not suggest which files to look at, which functions to change, or how to fix the code.\n\n"
        "ARCHITECTURE CONTEXT:\n"
        f"{arch}\n\n"
        f"{_SYSTEM_PROMPT_RULES}"
    )
